...
void ComputeWarpedHomoPoints(const Point3f & inputPixel, Point3f & outputPixel,const vector<float> & p);
Matx33f GetMatrix(const vector<float> & p);
Matx33f GetUnscaledMatrix(const vector<float> &p);
...
//2018-5-25 00:39:54 yxy 可能需要改动的地方；if cos(alpha) = t; sin(alpha) = sqrt(1-t) rotation [-pi pi]
inline void ComputeWarpedPixels(const int inputPixelx, const int inputPixely, int & outputPixelx, int & outputPixely, const vector<float> & p)
{
	...
	cv::Matx33f homographyMatrix = Homography::GetMatrix(p); //get Matrix函数，将8x1的vector转为3x3的homography matrix
	cv::Matx33f unNormalizedHomography  = scalingMatrix.inv() * homographyMatrix * scalingMatrix; //K^(-1)HK
