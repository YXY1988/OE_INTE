...
// KMYI: use GFT to initialize controlpoints 用lucascanade创建控制点网格板
...
//Scharr滤波器，比Sobel算子效果好，速度更快，在这里算了X和Y方向的图像梯度
...
void ShowDetailedOptimizationResults(const AlignmentResults & results, vector<float> parametersBaseline)
{
	...
	cout<< "exit flag  : "<< results.exitFlag<< "   (1->exceeded maxIter; 0->param converged; -1->residue converged)"<<endl;
...
void ShowConvergenceResults(Mat & templ, Mat &image, vector<vector<float> > &intermediateGuess, StructOfArray2di panelCorners)
{
	...
	namedWindow("Shot template", CV_WINDOW_NORMAL );
	AugmentFrameWithQuadrilater("Shot template", panelCorners,templ);
	waitKey(0);
	namedWindow("ConvergenceResults", CV_WINDOW_NORMAL );
	int x,y;
	for(uint i(0); i < 4; ++i)
	{
	...
	//	AugmentFrameWithQuadrilater("ConvergenceResults", warpedPixels, warpedImage);
	//	waitKey(50);
	//	//for(int i(0); i< 100000; ++i); //这句好像没用啊？
...
void WriteResultsOnImage(Mat & image, const AlignmentResults & results, int pixelsNumber, OptimizationType optimizationType)
{
	...
	putText(image, str.str(), Point(10,50), FONT_HERSHEY_TRIPLEX,0.7, CV_RGB(255,255,255));

	str.str("");
	str << "exit flag:" << results.exitFlag;
	putText(image, str.str(), Point(10,80), FONT_HERSHEY_DUPLEX, 0.5, CV_RGB(255,255,255));

	str.str("");
	str << "n. iterations:" << results.nIter;
	putText(image, str.str(), Point(10,110), FONT_HERSHEY_DUPLEX,0.5, CV_RGB(255,255,255));

	str.str("");
	str << "final residual norm:" << results.residualNorm.back();
	putText(image, str.str(), Point(10,140), FONT_HERSHEY_DUPLEX, 0.5, CV_RGB(255,255,255));

	str.str("");
	str << "number of pixels :" << pixelsNumber;
	putText(image, str.str(), Point(10,170), FONT_HERSHEY_DUPLEX, 0.5, CV_RGB(255,255,255));
}
