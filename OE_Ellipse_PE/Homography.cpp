...
{
  vector<float> newP = Homography::ParametersUpdateCompositional(p, deltap);//p_update = p + deltap
  // Check if the result is valid and update
  // TODO re-enable check with less restrictive parameters
   if (CheckHomography(newP)) //yxy 2018-5-21 23:04:48 打开注释，原先没有做 CheckHomography
    p = newP;
   else
...
//	float p3_1 = (1.0+p[3]);
  ...
  cout << "compositional check succeeded!" << endl;
  return true;
}
